...
/**
 * Copy of the DocumentationNormalizer that allows custom post resource models.
 */
final class OperationResourceClassDocumentationNormalizer implements NormalizerInterface, CacheableSupportsMethodInterface
{
    ...
    {
        if ($operation['resource_class'] ?? null) {
            $parameterResourceClass = $operation['resource_class'];
            $parameterResourceMetadata = $this->resourceMetadataFactory->create($parameterResourceClass);
        }
        ...
        switch ($method) {
            ...
            case 'POST':
                return $this->updatePostOperation($pathOperation, $mimeTypes, $operationType, $parameterResourceMetadata ?? $resourceMetadata, $parameterResourceClass ?? $resourceClass, $resourceShortName, $operationName, $definitions, $resourceMetadata, $resourceClass);
            case 'PATCH':
     ...
     * @return \ArrayObject
     */
    private function updatePostOperation(\ArrayObject $pathOperation, array $mimeTypes, string $operationType, ResourceMetadata $resourceMetadata, string $resourceClass, string $resourceShortName, string $operationName, \ArrayObject $definitions, ?ResourceMetadata $resultResourceMetadata = null, ?string $resultResourceClass)
    {
        ...
        $pathOperation['responses'] ?? $pathOperation['responses'] = [
            '201' => [
                'description' => sprintf('%s resource created', $resultResourceMetadata->getShortName()),
                'schema' => ['$ref' => sprintf('#/definitions/%s', $this->getDefinition($definitions, $resultResourceMetadata, $resultResourceClass,
                    $this->getSerializerContext($operationType, false, $resultResourceMetadata, $operationName)
                ))],
            ],
